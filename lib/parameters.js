...
const ParameterMissingError = require('./parameter-missing-error');

/**
 * Constants.
 */

const PRIMITIVE_TYPES = [Boolean, Number, String, function Null() {}];
...
Object.assign(Parameters, {
  ...
  _isPrimitive: function (value) {
    return PRIMITIVE_TYPES.some(function (Primitive) {
      return [typeof value, String(value)].some(function (val) {
        return val === Primitive.name.toLowerCase();
      });
    });
  },
  ...
  _permitObject: function (params, filters) {
    for (const key in filters) {
      ...
      if (_.isArray(filtersArray) && (param = this._fetch(key))) {
        ...
        if (
          _.isArray(param._params) ||
          (isArrObj = Object.keys(param._params).every(function (i) {
            return !_.isNaN(Number(i));
          }))
        ) {
          if (isArrObj) {
            params[key] = _.transform(
              param._params,
              function (result, value, key) {
            ...
            params[key] = param._params.map(function (param) {
              return param.permit(filtersArray).value();
            });
          } else {
            params[key] = param._params.filter(function (elem) {
              return Parameters._isPrimitive(elem);
            });
          }
          continue;
        }
